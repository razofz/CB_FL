...
states_oi <- c("Fetal Liver CS21-22", "Adult BM", "Fetal Liver CS17", "hPSC D31")
...
colnames(sample_table) <- c("sample_name", "sample_type", "timing", "cell_type", "proposed.sample.names")
...
#remove genes with lower mean expression than the lowest GOI to avoid having non-expressed genes affect the data later on
geneexp <- geneexp[rowMeans(geneexp) >= min(rowMeans(geneexp[rownames(geneexp) %in% GOI$gene_name,])),]
...
pcatable <- data.frame(PC1 = pca$x[, 1], PC2 = pca$x[, 2], PC3 = pca$x[, 3], sample_table)


ggplot(data = pcatable, aes_string(x = "PC1", y = "PC2", color = "sample_type", shape = "cell_type")) + 
  geom_point(size = 4) + theme_bw() + coord_fixed() + scale_color_manual(values = sample_colors) +
...
#Check which genes overlap between top 500 variable and fetal core genes, should be 0
...
for (i in 1:5) {
  ...
  write.csv(rownames(pca$rotation), file = paste0('20221028_random_PCA_genes_pseudoreps_core_', i, '.csv'))
